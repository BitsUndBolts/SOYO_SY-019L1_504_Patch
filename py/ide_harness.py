...
Known limitation: no PIC/interrupt modelling, so IRQ-driven waits run their
slow worst-case path -- compare against the original ROM under identical
inputs rather than judging the patched ROM alone.

Typical use:
    from ide_harness import *
    e = Emu(load_rom(PATCHED_ROM), 16000, 16, 63)   # simulated drive C/H/S
    r = e.call(0x0800, 0, 0x0080)                    # INT 13h AH=08h, DL=80h
    r = e.call(0x0201, 0x0001, 0x0080)               # read LBA 0
    print(r['ide'])                                  # task-file writes + LBA
"""
import os, struct, sys
from unicorn import *
from unicorn.x86_const import *
ROOT        = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
ORIG_ROM    = os.path.join(ROOT, 'binary', 'SY019L1_27C512_ORIGINAL.BIN')
PATCHED_ROM = os.path.join(ROOT, 'binary', 'SY019L1_27C512_CHSPATCH.BIN')
def load_rom(path): return open(path, 'rb').read()
...
class Emu:
    ...
    def call(s,ax,cx,dx,bx=0x1000,es=0x2000,e32=0):
        ...
        regs['n']=s.n; regs['stack_depth']=0x100-s.minsp
        return regs
