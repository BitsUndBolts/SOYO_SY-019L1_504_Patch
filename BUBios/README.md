...
**Status:** experimental. So far it has been tested only in the emulator; it has not run on real hardware yet. Keep the ECHS EPROM (MD5 `8e520e91…`) and the original dump ready as a fallback.

| | |
|---|---|
| ROM to burn | `binary/BUBIOS_SY019L1.BIN` (64 KB, 27C512) |
| MD5 | `99a0d306a1ffa42634b81a997cc62e1c` |
| Based on | `../binary/SY019L1_27C512_CHSPATCH.BIN` (ECHS v5, MD5 `8e520e91100f932d3f054883b08d37da`) |
| New code / data | 1273 B at `DA84` (131 B free), 1369 B at `7902` (163 B free) |
...
| Where | Key | Action |
|---|---|---|
| Summary / Tools / Exit | ←/→ or Tab / Shift-Tab | Switch tab |
| | ↑/↓ | Select an entry |
| | Enter | Run the selected entry |
| | ESC | Go to Summary (ESC on Summary goes to Exit) |
| | F10 | Save and exit |
| | F2/F3 | Change the colour scheme |
| Standard / Advanced / Chipset | Tab / Shift-Tab | Next / previous tab (the arrow keys still move between fields, as in AMI) |
| | ESC | Back to Summary |
...
| `08D5` | `CALL bu_stdkey` + `NOP`: Tab/Shift-Tab inside Standard CMOS |
...
**Removed from the menu:** the AMI "Improper use of setup" warning screen, and the low-level **Hard Disk Utility** (format, interleave, media analysis). The utility is meaningless on IDE and CF drives. Its code is still in the ROM, and adding one list entry would bring it back.
...
python3 py/test_bubios.py      # 47 behavioural checks of the setup in the emulator
...
- `test_bubios.py`: 47/47 pass. It covers the Summary contents for five drive geometries, every tab transition, editing a value and seeing it on the Summary page, F10 + Y writing CMOS with a valid checksum, quitting without saving leaving CMOS untouched, colour cycling, and loading defaults.
- `regress_echs.py`: all 10 ECHS geometries pass, and INT 19h boot passes for all 5 cases, exactly as with the ECHS ROM.

## Please check on the real board

1. The **CPU clock** reading. This is the one value the emulator cannot confirm. Please report what it shows, with the turbo switch on and off.
2. **Tab and Shift-Tab** inside the Standard, Advanced and Chipset pages.
3. **Save and exit**, then boot from the 8 GB CF card. The disk code is untouched, but this is the final proof.
4. The **colour scheme** at first start comes from your CMOS (register 37h). If it isn't teal, press F2 or F3.
5. Password and auto-detect (Tools tab).
